...
DATA_DIR = _env_first("ALA_DATA_DIR", "ODYSSEUS_DATA_DIR", default=get_default_data_dir())

# Data file paths
# Single source of truth: every persisted file/dir lives under DATA_DIR, which
# is the ONLY place ALA_DATA_DIR / ODYSSEUS_DATA_DIR are read. Import these constants instead of
...
MAIL_ATTACHMENTS_DIR = _env_first(
    "ALA_MAIL_ATTACHMENTS_DIR",
    "ODYSSEUS_MAIL_ATTACHMENTS_DIR",
    default=os.path.join(DATA_DIR, "mail-attachments"),
)
...
def internal_api_base() -> str:
    """Base URL for in-process loopback calls to ALA's own API.

    Agent tools and background jobs reach admin-gated routes by calling the
    running server over HTTP. Resolution order:
      1. ALA_INTERNAL_BASE / ODYSSEUS_INTERNAL_BASE - explicit override (e.g. behind a TLS proxy).
    ...
    """
    override = os.environ.get("ALA_INTERNAL_BASE") or os.environ.get("ODYSSEUS_INTERNAL_BASE")
    if override:
