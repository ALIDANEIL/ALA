...
def create_default_admin():
    ...
    try:
        ...
        username = (os.getenv("ALA_ADMIN_USER") or os.getenv("ODYSSEUS_ADMIN_USER") or "").strip().lower()
        password = (os.getenv("ALA_ADMIN_PASSWORD") or os.getenv("ODYSSEUS_ADMIN_PASSWORD") or "").strip()

        if username and password:
            # Both provided via env — validate before using
            if username in RESERVED_USERNAMES:
                print(f"  [error] ALA_ADMIN_USER '{username}' is a reserved username")
                return "failed"
            if len(password) < PASSWORD_MIN_LENGTH:
                print(f"  [error] ALA_ADMIN_PASSWORD must be at least {PASSWORD_MIN_LENGTH} characters")
                return "failed"
        elif sys.stdin.isatty() and not (os.getenv("ALA_SKIP_ADMIN_PROMPT") or os.getenv("ODYSSEUS_SKIP_ADMIN_PROMPT")):
        ...
        if sys.stdin.isatty() and not (os.getenv("ALA_ADMIN_PASSWORD") or os.getenv("ODYSSEUS_ADMIN_PASSWORD")):
            print(f"  [ok] Admin account created ({username})")
        else:
            print(f"  [ok] Initial admin user created ({username})")
            if not (os.getenv("ALA_ADMIN_PASSWORD") or os.getenv("ODYSSEUS_ADMIN_PASSWORD")):
                print(f"        Temporary password: {password}")
                print(f"        ** Change it after first login. Set ALA_ADMIN_PASSWORD to choose your own. **")
        return "created"
...
def main():
    ...
    # Load .env so pre-seeded ALA_ADMIN_USER / ALA_ADMIN_PASSWORD (and
    ...
    if not (os.getenv("ALA_SKIP_RUN_HINT") or os.getenv("ALA_SKIP_RUN_HINT")):
